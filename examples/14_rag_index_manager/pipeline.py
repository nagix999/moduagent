...
class RAGIndexManager:
    ...
    async def preview(self, *, force_rebuild: bool = False) -> SyncReport:
        ...
        async with self._lock:
            ...
            sources, plan = self._scan_and_plan(force_rebuild=force_rebuild)
            ...
            try:
                ...
                for action in plan.actions:
                    if action.change is ChangeKind.UNCHANGED:
                        ...
                        self.catalog.carry_forward_document(run_id, action.previous)
                        continue
                    ...
                    chunks = await self._materialize_chunks(action)
                    ...
                    await self._embed_and_upsert(staging, chunks)
                ...
                if not expected_ids:
                    raise PipelineError("staging manifest contains no retrieval chunks")
        ...
        async with self._lock:
            ...
            try:
                previous = self.catalog.rollback_to_generation(
                    self.config.kb_id, target
                )
            except BaseException:
                await self.vector_store.rollback()
                raise
            if previous != current or published.previous_collection is None:
                raise PipelineError("rollback predecessor does not match active state")
            documents = self.catalog.list_documents(self.config.kb_id)
            return SyncReport(
                operation="rollback",
                status="rolled_back",
    ...
    async def _materialize_chunks(self, action: SyncAction) -> tuple[Chunk, ...]:
        source = action.source
        if source is None:
        ...
        if stage is ProcessingStage.PARSE:
            ...
            if parsed.parser_fingerprint != self.pipeline.parser:
                raise PipelineError("Docling parser revision changed during the run")
            self.artifacts.save_docling(source, parsed)
            document = parsed.document_json
        if document is None:
            raise PipelineError("no Docling artifact is available for restructuring")
        ...
        if self.pipeline.layout_refinement is not None:
            ...
            if not refinement_loaded and stage in {
                ...
                if not isinstance(patches, tuple):
                    raise PipelineError("layout refiner must return an ordered tuple")
    ...
    async def _embed_and_upsert(
        ...
        for start in range(0, len(chunks), self.config.embedding_request_size):
            ...
            inserted = await self.vector_store.upsert(staging, batch_chunks, embedded)
